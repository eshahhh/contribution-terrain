...
export class TerrainSvgGenerator {
    ...
    private readonly lightDir = this.normalize3({ x: 0.3, y: -0.3, z: 1.0 });

    private readonly smoothSigma = 0.6;
    private readonly radialInfluence = 2.5;
    private readonly contoursEnabled = false;
    private readonly contourLevels: number[] = [];
    private readonly noiseScale = 0.3;
    private readonly noiseFrequency = 0.4;

    private calculateQuartileThresholds(heightField: number[][]): QuartileThresholds {
    ...
    generateSvg(
        ...
        const bounds = this.calculateBoundsGeneric(
            grid[0]?.length || 0,
            grid.length,
            maxEffectiveCount,
            rotationAngle
    ...
    private buildHeightField(grid: number[][]): number[][] {
        const days = grid.length;
        const weeks = grid[0]?.length || 0;
        const field: number[][] = Array.from({ length: days }, (_, y) =>
            Array.from({ length: weeks }, (_, x) => grid[y][x] || 0)
        );
        return field;
    }

    private gaussianSmooth(field: number[][], sigma = 1.0): number[][] {
        if (!sigma || sigma <= 0) return field.map(row => row.slice());
        ...
        const preservedField: number[][] = Array.from({ length: h }, (_, y) =>
        ...
        const temp: number[][] = Array.from({ length: h }, () => Array(w).fill(0));
        for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
                let acc = 0;
                let weightSum = 0;
    ...
    private simpleNoise(x: number, y: number, frequency: number = 1.0): number {
    ...
    private addTerrainNoise(field: number[][], originalField: number[][]): number[][] {
        const days = field.length;
        const weeks = field[0]?.length || 0;
        const noisy: number[][] = field.map(row => row.slice());
    ...
    private applyRadialInfluence(field: number[][]): number[][] {
        const days = field.length;
        const weeks = field[0]?.length || 0;
        const influenced: number[][] = field.map(row => row.slice());
            ...
            for (let x = 0; x < weeks; x++) {
                ...
                for (let dy = -radius; dy <= radius; dy++) {
                    for (let dx = -radius; dx <= radius; dx++) {
                        const ny = y + dy;
                        const nx = x + dx;
                        if (ny < 0 || ny >= days || nx < 0 || nx >= weeks) continue;
    ...
    private preserveZeroValleys(smoothed: number[][], original: number[][]): number[][] {
    ...
    private calculateDynamicContours(maxCount: number): number[] {
    ...
    private applyQuartileHeightScaling(count: number, thresholds: QuartileThresholds): number {
        if (count <= 0) return 0;

        const baseHeight = 8;
        const heightLevels = [0, baseHeight, baseHeight * 2, baseHeight * 3.5, baseHeight * 6];

        if (count > 0 && count < thresholds.minNonZero) return baseHeight * 0.5;
        if (count <= thresholds.q1) return heightLevels[1];
        if (count <= thresholds.q2) return heightLevels[2];
        if (count <= thresholds.q3) return heightLevels[3];
        return heightLevels[4];
    }

    private renderTerrain(smoothedCounts: number[][], rotationAngle: number, maxCount?: number, scalingThresholds?: QuartileThresholds): string[] {
        const facesWithDepth: TriangleWithDepth[] = [];
        const days = smoothedCounts.length;
        const weeks = smoothedCounts[0]?.length || 0;

        const renderOrder: Array<{ y: number; x: number }> = [];
        for (let y = 0; y < days - 1; y++) {
            for (let x = 0; x < weeks - 1; x++) renderOrder.push({ y, x });
        }
        ...
        for (const { y, x } of renderOrder) {
            ...
            const g00: Point3D = { x: x0, y: y0, z: 0 };
            const g10: Point3D = { x: x1, y: y0, z: 0 };
            const g01: Point3D = { x: x0, y: y1, z: 0 };
            const g11: Point3D = { x: x1, y: y1, z: 0 };

            const t00: Point3D = { x: x0, y: y0, z: Math.max(0, h00Varied) };
            const t10: Point3D = { x: x1, y: y0, z: Math.max(0, h10Varied) };
            const t01: Point3D = { x: x0, y: y1, z: Math.max(0, h01Varied) };
            const t11: Point3D = { x: x1, y: y1, z: Math.max(0, h11Varied) };
    ...
    private renderBaseGrid(grid: number[][], rotationAngle: number): string[] {
        const baseGridElements: string[] = [];
        const days = grid.length;
        const weeks = grid[0]?.length || 0;
        ...
        const cellsWithDepth: TriangleWithDepth[] = [];
        ...
        for (let y = 0; y < days; y++) {
            for (let x = 0; x < weeks; x++) {
                ...
                const corner1: Point3D = { x: x0, y: y0, z: baseHeight };
                const corner2: Point3D = { x: x1, y: y0, z: baseHeight };
                const corner3: Point3D = { x: x1, y: y1, z: baseHeight };
                const corner4: Point3D = { x: x0, y: y1, z: baseHeight };
    ...
    private addBaseTriangleWithDepth(
        facesWithDepth: TriangleWithDepth[],
        tri: [Point3D, Point3D, Point3D],
        color: string,
    ...
    private addTriangleWithDepthSorting(
        ...
        count: number,
        maxCount?: number,
        scalingThresholds?: QuartileThresholds,
        rotationAngle?: number
    ): void {
        const normal = this.computeNormal(tri[0], tri[1], tri[2]);
        const nl = Math.max(0, this.dot3(normal, this.lightDir));
        ...
        if (scalingThresholds && avgHeight > 0) {
            if (avgHeight >= 48) colorCount = scalingThresholds.max;
            else if (avgHeight >= 28) colorCount = scalingThresholds.q3;
            else if (avgHeight >= 16) colorCount = scalingThresholds.q2;
            else if (avgHeight >= 8) colorCount = scalingThresholds.q1;
            else colorCount = scalingThresholds.minNonZero;
        }
    ...
    private renderContours(field: number[][], levels: number[], rotationAngle: number, maxCount?: number): string[] {
        const days = field.length;
        const weeks = field[0]?.length || 0;
        const lines: string[] = [];
        ...
        for (const level of levels) {
            ...
            const segments: Array<{ a: Point3D; b: Point3D }> = [];
            for (let y = 0; y < days - 1; y++) {
                for (let x = 0; x < weeks - 1; x++) {
                    ...
                    const interp = (hA: number, hB: number, tA: Point3D, tB: Point3D): Point3D => {
                    ...
                    const p00: Point3D = { x: x * this.cellSize, y: y * this.cellSize, z: h00 };
                    const p10: Point3D = { x: (x + 1) * this.cellSize, y: y * this.cellSize, z: h10 };
                    const p11: Point3D = { x: (x + 1) * this.cellSize, y: (y + 1) * this.cellSize, z: h11 };
                    const p01: Point3D = { x: x * this.cellSize, y: (y + 1) * this.cellSize, z: h01 };

                    const edgePoints: Point3D[] = [];
                    if ((c0 ^ c1) === 1) edgePoints.push(interp(h00, h10, p00, p10));
                    if ((c1 ^ c2) === 1) edgePoints.push(interp(h10, h11, p10, p11));
                    if ((c2 ^ c3) === 1) edgePoints.push(interp(h11, h01, p11, p01));
                    if ((c3 ^ c0) === 1) edgePoints.push(interp(h01, h00, p01, p00));
    ...
    private computeNormal(a: Point3D, b: Point3D, c: Point3D): { x: number; y: number; z: number } {
    ...
    private dot3(a: { x: number; y: number; z: number }, b: { x: number; y: number; z: number }) {
        return a.x * b.x + a.y * b.y + a.z * b.z;
    }

    private getColor(count: number, brightness: number, maxCount?: number, scalingThresholds?: QuartileThresholds): string {
        ...
        if (count <= 0) {
            ...
            switch (heightLevel) {
                case 0: baseColor = '#c3e4c8ff'; break;
                case 1: baseColor = '#9be9a8'; break;
                case 2: baseColor = '#40c463'; break;
                case 3: baseColor = '#30a14e'; break;
                case 4: baseColor = '#216e39'; break;
                default: baseColor = '#216e39'; break;
            }
        } else {
    ...
    private getHeightLevel(count: number, thresholds: QuartileThresholds): number {
        if (count <= 0) return 0;
        if (count < thresholds.minNonZero) return 0;
        if (count <= thresholds.q1) return 1;
        if (count <= thresholds.q2) return 2;
        if (count <= thresholds.q3) return 3;
        if (count <= thresholds.max) return 4;
        return 5;
    }
    ...
    private getContourColor(level: number, maxLevel: number): string {
    ...
    private calculateBoundsGeneric(weeks: number, days: number, maxCount: number, rotationAngle: number): { width: number; height: number } {
