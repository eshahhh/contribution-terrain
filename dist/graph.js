export class GraphSvgGenerator {
    ...
    renderGrid(grid, rotationAngle = this.rotationAngle) {
        const baseTiles = [];
        const columns = [];
        const renderOrder = [];
    ...
    renderBaseTile(weekIndex, dayIndex, rotationAngle = this.rotationAngle) {
    ...
    renderColumn(weekIndex, dayIndex, count, grid, rotationAngle = this.rotationAngle) {
        if (count <= 0)
            return '';
    ...
    calculateAmbientOcclusion(weekIndex, dayIndex, grid) {
    ...
    getStrokeColor(count, brightness) {
        if (brightness > 0.8)
            return '#2a2a2a';
        if (brightness > 0.6)
            return '#333333';
    ...
    calculateBounds(grid, rotationAngle = this.rotationAngle) {
